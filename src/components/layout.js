        ...
        <div class="flex justify-between items-center w-full px-6 py-4">
            ...
            <div class="flex items-center gap-4">
                <div class="hidden md:flex items-center gap-6 mr-6">
                    <button class="nav-link text-primary font-semibold cursor-pointer" data-route="dashboard">Painel</button>
                    <button class="nav-link text-slate-500 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors px-2 py-1 rounded cursor-pointer" data-route="schedule">Agenda</button>
                    <button class="nav-link text-slate-500 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors px-2 py-1 rounded cursor-pointer" data-route="staff">Equipa</button>
                </div>
...
export function renderNavbar(activeRoute = 'dashboard') {
    const isActive = (route) => route === activeRoute ? 'text-primary bg-primary/10 rounded-xl' : 'text-slate-500 dark:text-slate-400 hover:text-primary transition-colors';
    ...
    <nav class="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 pb-6 pt-2 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md border-t border-slate-200/50 dark:border-slate-800/50 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] z-50 rounded-t-xl">
        <button class="nav-btn flex flex-col items-center justify-center px-4 py-1 ${isActive('dashboard')}" data-route="dashboard">
            ...
            <span class="font-inter text-[11px] font-semibold tracking-wide uppercase mt-1">Painel</span>
        </button>
        <button class="nav-btn flex flex-col items-center justify-center px-4 py-1 ${isActive('schedule')}" data-route="schedule">
            ...
            <span class="font-inter text-[11px] font-semibold tracking-wide uppercase mt-1">Agenda</span>
        </button>
        <button class="nav-btn flex flex-col items-center justify-center px-4 py-1 ${isActive('staff')}" data-route="staff">
            ...
            <span class="font-inter text-[11px] font-semibold tracking-wide uppercase mt-1">Equipa</span>
        </button>
        <button class="nav-btn flex flex-col items-center justify-center px-4 py-1 ${isActive('more')}" data-route="more">
            <span class="material-symbols-outlined">more_horiz</span>
            <span class="font-inter text-[11px] font-semibold tracking-wide uppercase mt-1">Mais</span>
        </button>
    </nav>`;
}
